...
export const SIMILAR_INGREDIENTS = {
	...
	"queso panela": [
		...
		"queso panela para espolvorear",
		"queso panela rebanado",
...
export const INGREDIENT_EQUIVALENTS = {
	Verduras: [
		...
		"1 pza de calabacita",
		"1/2 tza de cebolla",
		"1/2 tza de champifión",
		"1/2 pza de chayote",
		"1/4 tza de chicharos",
		"2 tzas de coliflor",
		"2 tzas de espinacas",
		"1/2 tza de jícama",
		"1 pza de jitomate",
		...
		"1 tza de pimiento",
		"1 tza de rábano",
		"1/2 tza de zanahoria",
		"4 pzas de mini zanahorias",
		"2 tzas de lechuga",
		"3 pzas de col de bruselas",
		"6 pzas de espárragos",
	],
	"Alimentos de origen animal": [
		"30 gr de carne de res",
		"35 gr de salmón",
		"30 gr de atún fresco",
		"1/3 lata de atún en conserva",
		"1/2 pza de chuleta",
		"2 pzas de clara de huevo",
		"75 gr de pescado fileteado",
		"35 gr de pollo sin piel",
		"2 reb de jamón de pavo",
		"50 gr de queso cottage",
		"40 gr de queso panela",
		"1 pza de huevo entera",
	],
	Frutas: [
		"1/2 tza de arándano",
		"7 pzas de ciruela pasa",
		"2 pzas de durazno",
		"1 tza de fresas",
		"3 pzas de guayaba",
		"2 pzas de mandarina",
	...
	"Cereales y tubérculos": [
		...
		"1/2 tza de avena",
		"1/3 pza de camote",
		"1/3 tza de pasta cocida",
		'1 paq de galletas horneadas "Salmas"',
		"2 1/2 tza de palomitas naturales",
		"3 cdas de granola baja en grasa",
		"20 gr de quinoa",
		"1 reb de pan integral",
		"1/2 pza de papa",
		"2 pzas de tortilla de nopal",
	],
	Leguminosas: [
		"1/2 tza de frijol cocido",
		"1/2 tza de garbanzo cocido",
		"1/2 tza de lenteja cocida",
		"1/3 tza de soya cocida",
		"5 cdas de hummus",
	],
	Leche: [
		"1 tza de leche descremada",
		"1 tza de yogurt bajo en grasa sin azúcar",
		"1/4 tza de yogurt griego sin azúcar",
		"1 tza de bebida de soya (leche de soya)",
	],
	"Aceites y grasas (sin proteína)": [
		"1 cda de aceite",
	...
	"Aceites y grasas (con proteína)": [
		...
		"1 tza de bebida de almendra (leche de almendra)",
	],
};
...
export function normalizeIngredientName(name) {
	...
	for (const [mainIngredient] of Object.entries(SIMILAR_INGREDIENTS)) {
...
/**
 * Fingerprint for meal dedupe during migration.
 * @param {{ name: string, ingredients?: { name: string, quantity?: string | object }[] }} meal
 */
		...
		.map((i) => {
			const q =
				typeof i.quantity === "string"
					? i.quantity
					: i.quantity?.raw || formatQuantity(i.quantity) || "";
			return `${normalizeIngredientName(i.name)}:${q}`;
		})
		.sort()
		.join("|");
	return `${name}::${ings}`;
...
/**
 * Build a v2 Ingredient from a legacy `{ name, quantity: string }` blob.
 * @param {{ name: string, quantity: string }} legacy
 */
export function ingredientFromLegacy(legacy) {
	return {
		id: createId(),
		name: legacy.name,
		quantity: parseQuantity(legacy.quantity),
	};
}

/**
 * Legacy shopping helpers (sum + format list of parsed quantities).
 */
export function sumIngredients(ingredients) {
	...
	ingredients.forEach((ingredient) => {
		...
		if (!summed[normalizedName]) {
			...
			summed[normalizedName].quantities.push(legacyParsed);
			if (!summed[normalizedName].variations.includes(ingredient.name)) {
				summed[normalizedName].variations.push(ingredient.name);
			}
			const isNewSource = !summed[normalizedName].sources.some(
				(s) => s.meal === ingredient.mealName && s.day === ingredient.day,
			);
			if (isNewSource) {
	...
	return Object.entries(grouped)
		...
		.map(([unit, value]) => {
			...
			if (value % 1 !== 0) {
				...
				else if (Math.abs(fraction - 0.67) < 0.02) fractionStr = "2/3";
				if (fractionStr) {
					return whole > 0
						? `${whole} ${fractionStr} ${unit}`
						: `${fractionStr} ${unit}`;
				}
			}
			return `${value} ${unit}`;
